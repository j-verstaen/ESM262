...
# I'll start on the second day ...why? because need to reference a day before for it to work
...
# the averge of 3 days (1 before, current and after) of daily maximum tempeature is greater than 20
# 0.5 if average is greater than 15
# 0 otherwise
...
clim$tindex = rep(NA, times=nrow(clim))

last_row = nrow(clim)-1

#i = 3

for ( i in 2:last_row) {
  
  clim$tindex[i] = ifelse(clim$rain[i] > 0, 0,
                         
                        ifelse(mean(clim$tmax[i], clim$tmax[i-1], clim$tmax[i+1]) > 20, 1,
                               
                              ifelse(mean(clim$tmax[i], clim$tmax[i-1], clim$tmax[i+1]) < 15, 0, 
...
##naomi did it differently, she seperated out each thing
#clim$risk[i] = ifelse(clim$tavg[i]) > 15, 1, 0)
#clim$risk[i] = ifelse(clim$tavg[i] <10 & clim$tavg[i] >15, 0.5, clim$risk[i])
#clim$risk[i] = ifelse(clim$rain[i] > 0, 0, clim$risk[i], 0)
...
computediversity_mod(catch1)
computediversity_mod(catch2)
